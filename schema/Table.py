...
class Table:
        ...
        def dropIndexes (self):
                ...
                for index in list(self.indexes.keys()):
        ...
        def getIndexCreationStatements (self):
                stmts = []
                for (index, stmt) in list(self.indexes.items()):
        ...
        def dropKeys (self):
                ...
                for keyCol in list(self.fKeys.keys()):
        ...
        def getForeignKeyCreationStatements (self):
                ...
                stmts = []
                for (keyCol, (fkTable, fkKeyCol)) in list(self.fKeys.items()):
        ...
        def getCommentStatements (self):
                ...
                if TABLE in self.comments:
                        stmts.append (cmd % ('TABLE', self.name,
                                self.comments[TABLE]) )

                if COLUMN in self.comments:
                        for (col, comment) in list(self.comments[COLUMN].items()):
                                colName = '%s.%s' % (self.name, col)
                                stmts.append (cmd % ('COLUMN', colName,
                                        comment))

                if INDEX in self.comments:
                        for (idx, comment) in list(self.comments[INDEX].items()):
