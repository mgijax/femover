...
tableName = 'phenotable_disease_cell'

# MySQL statement to create this table
createStatement = '''CREATE TABLE %s  ( 
	phenotable_disease_cell_key		int		not null,
	...
	PRIMARY KEY(phenotable_disease_cell_key))''' % tableName

# Maps from index suffix to create statement for that index.  In each
# statement, the first %s is for the index name, and the second is for the
# table name.
indexes = {
	'disease_key' : 'create index %s on %s (phenotable_disease_key)',
}
...
comments = {
	...
	Table.COLUMN : {
		'phenotable_disease_cell_key' : 'unique key identifying this row',
		'phenotable_disease_key' : 'key for the corresponding disease',
		'call' : 'Y, N, or NA',
		'sex' : 'M, F, or NA',
