...
def _vocabNameSortKey (a):
        # sort key for this vocabulary name (case-insensitive; None sorted before non-None)

        if a is None:
                return (-1, ' ')
        return (0, a.lower())

def _dagNameSortKey (a):
        # sort key for this DAG name (case-insensitive; None sorted before non-None; with
        # special ordering for the three GO DAG names)
...
def _prepareOrderingDictionary (cmd, fn):
        ...
        for row in rows:
                ...
                if name in nameToKeys:
                        nameToKeys[name].append (row[keyCol])
                else:
                        nameToKeys[name] = [ row[keyCol] ]

        names = list(nameToKeys.keys())       # list of all names
        names.sort(key=fn)
...
def _termSortKey(a):
        # return a sort key for tuple (vocabCol, seqNum, term, termKey)
        vocab, seqNum, term, termKey = a
        if vocab is None:
                vocab = ' '
        if seqNum is None:
                seqNum = -1
        if term is None:
                term = ' '
        return (vocab, seqNum, term, termKey)
...
class VocabSorter:
        ...
        def __getTermKeysAndDefaultOrder (self):
                ...
                termList.sort(key=_termSortKey)
        ...
        def __getDAGs (self):
                ...
                for row in rows:
                        ...
                        if parent not in self.notRoots:
        ...
        def __orderChildren (self):
                ...
                parentKeys = list(self.children.keys())
        ...
        def __orderDagRoots (self):
                ...
                for (termKey, dagKey) in list(self.roots.items()):
        ...
        def __computeFinalOrder (self):
                ...
                while stack:
                        ...
                        # order
                        if toDo not in self.finalOrder:
                                seqNum = seqNum + 1
                ...
                termKeys = self.__orderTermList (list(self.defaultOrder.keys()))
                for term in termKeys:
                        if term not in self.finalOrder:
                                seqNum = seqNum + 1
        ...
        def __getTransitiveClosure (self):
                ...
                for row in rows:
                        ...
                        if descendent not in self.ancestors:
...
class VocabSorterAlpha:
        ...
        def _loadData (self):
                ...
                vocabSeqNum = _prepareOrderingDictionary (cmd1, _vocabNameSortKey)
                ...
                dagSeqNum = _prepareOrderingDictionary (cmd2, _dagNameSortKey)
                ...
                termSeqNum = _prepareOrderingDictionary (cmd3, symbolsort.splitter)
...
def setVocabs (vocabKeys):
        ...
        if type(vocabKeys) == type([]):
                ...
                raise Exception('error: unknown param in setVocabs: %s' % vocabKeys)
