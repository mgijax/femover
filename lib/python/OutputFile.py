...
class OutputFile:
        ...
        def writeToFile (self, fieldOrder, columns, rows):
                ...
                if not self.isOpen:
                        raise Exception('%s: %s' % (error, ClosedFile))
                ...
                if not self.columnCount:
                        ...
                        raise Exception('%s: %s' % (error, ColumnMismatch % (self.columnCount,
                                len(fieldOrder))) )
...
class CachingOutputFileFactory:
        ...
        def closeAll (self):
                ...
                for num in list(self.outputFiles.keys()):
...
def clean(dirtystring):
        ds = ''
        for x in dirtystring:
                if x in validCharacters:
                        ds = ds + x
        dirtystring = ds
