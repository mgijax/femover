...
def keepNots(flag = True):
	...
	global KEEP_NOTS
	if BUILT_KEEPER_TABLE:
		raise error, 'Call keepNots() before other GOFilter functions.'
...
def removeAllND(flag = True):
	...
	if BUILT_KEEPER_TABLE:
		raise error,  \
			'Call removeAllND() before other GOFilter functions.'
...
def getAnnotationCount (markerKey):
	...
	if BY_MARKER.has_key(markerKey):
...
def getAnnotationCountForRef (refsKey):
	...
	if BY_REFERENCE.has_key(refsKey):
