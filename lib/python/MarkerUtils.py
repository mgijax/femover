...
MA_TABLE = None		# name of already-computed table of marker and allele
			# ...pairs (including expresses component)
...
def getMarkerAlleleTable():
	# get the name of a temp table that has been populated with marker /
	# allele pairs, including via expresses component relationships
	...
	cmd0 = '''create temporary table %s (
		...
		_Allele_key int not null)''' % MA_TABLE
	...
	cmd1 = '''insert into %s
		...
		where _Category_key = %d''' % (MA_TABLE, ec)

	cmd2 = 'create index p1 on %s (_Marker_key)' % MA_TABLE
	cmd3 = 'cluster %s using p1' % MA_TABLE
	cmd4 = 'create index p2 on %s(_Allele_key)' % MA_TABLE
	cmd5 = 'select count(1) from %s' % MA_TABLE

	for cmd in [ cmd0, cmd1, cmd2, cmd3, cmd4, cmd5 ]:
		cr = dbAgnostic.execute(cmd)

	logger.debug('Filled %s with %d rows' % (MA_TABLE, cr[1][0][0]))
