...
def getHybridClusterKeys(markerKey):
	...
	if not CACHED_SOURCES.has_key(HYBRID):
		_loadSource(HYBRID) 
...
def getHomoloGeneClusterKeys(markerKey):
	...
	if not CACHED_SOURCES.has_key(HOMOLOGENE):
...
def getHgncClusterKeys(markerKey):
	...
	if not CACHED_SOURCES.has_key(HGNC):
		_loadSource(HGNC) 
...
def getSource(clusterKey):
	...
	if CLUSTER_SOURCES.has_key(clusterKey):
...
def getSourceOfCluster(hybridClusterKey):
	...
	if not CACHED_SOURCES.has_key('derived'):
		_loadSourceClusters()
...
def getMarkersPerCluster():
	...
	for markerKey in HYBRID_CLUSTERS.keys():
...
def _loadSourceClusters():
	...
	if CACHED_SOURCES.has_key('derived'):
		return
...
def _loadSource(sourceKey):
	...
	if CACHED_SOURCES.has_key(sourceKey):
		return
