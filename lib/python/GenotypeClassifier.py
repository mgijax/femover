...
def sortKey (t):
        # generate sort key for t, which has: (allele key, sequence num, genotype key)
        # sorting rules:
        ...
        (alleleKey, seqNum, genotypeKey) = t

        annotationFlag = 1
        if genotypeKey in DISEASE_MODEL or genotypeKey in HAS_PHENO_DATA:
                annotationFlag = -1

        typeFlag = TYPES.index(GENOTYPE_TYPES[genotypeKey])

        return (alleleKey, annotationFlag, typeFlag, seqNum, genotypeKey)
...
def getClass (genotypeKey):
        ...
        if genotypeKey in GENOTYPE_TYPES:
...
def getDesignation (alleleKey, genotypeKey):
        ...
        if (alleleKey, genotypeKey) in DESIGNATION:
...
def getSequenceNum (alleleKey, genotypeKey):
        ...
        if (alleleKey, genotypeKey) in SEQUENCE_NUM:
...
def getAlleleGenotypeKey (alleleKey, genotypeKey):
        ...
        if (alleleKey, genotypeKey) in AG_KEY:
...
def hasPhenoData (genotypeKey):
        ...
        if genotypeKey in HAS_PHENO_DATA:
...
def isDiseaseModel (genotypeKey):
        ...
        if genotypeKey in DISEASE_MODEL:
