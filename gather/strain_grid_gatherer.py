...
def genotypeSortKey(a):
        # get a key for sorting genotype data (allele pairs, genotype key)
        return (symbolsort.splitter(a[0]), a[1])
...
class StrainGridGatherer (Gatherer.CachingMultiFileGatherer):
        ...
        def sortHeaders(self):
                ...
                headers.sort(key=symbolsort.splitter)
                i = 0
        ...
        def sortTerms(self):
                ...
                allTerms = list(self.termKey.keys())
                allTerms.sort(key=symbolsort.splitter)
                i = 0
        ...
        def sortGenotypes(self):
                ...
                toSort.sort(key=genotypeSortKey)
                i = 0
        ...
        def addEmptyGridCells(self):
                ...
                for (gridCellKey, strainKey, headingKey) in sorted(list(self.gcCount.keys())):
                ...
                i = 0
                for strainKey in sorted(list(toAdd.keys())):
        ...
        def buildGridCellTable(self):
                ...
                for (trio, count) in sorted(list(self.gcCount.items())):
        ...
        def addEmptyPopupCells(self, rowsPerGridCell, columnsPerGridCell, definedCells):
                ...
                i = 0
                for gridCellKey in sorted(list(rowsPerGridCell.keys())):
        ...
        def buildPopupTables(self):
                ...
                for (pair, count) in sorted(list(self.popCount.items())):
