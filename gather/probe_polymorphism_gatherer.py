...
def comparePolymorphisms (a, b):
	# sort by jnum, endonuclease, and symbol
	
	j = cmp(a[jnumCol], b[jnumCol])
	if j:
		return j

	if a[endonucleaseCol] != None:
		if b[endonucleaseCol] != None:
			e = symbolsort.nomenCompare(a[endonucleaseCol], b[endonucleaseCol])
			if e:
				return e
		else:
			return -1
	elif b[endonucleaseCol] != None:
		return 1

	return symbolsort.nomenCompare(a[symbolCol], b[symbolCol])
...
class ProbePolymorphismGatherer (Gatherer.Gatherer):
	...
	def postprocessResults(self):
		...
		self.finalResults.sort(comparePolymorphisms)
