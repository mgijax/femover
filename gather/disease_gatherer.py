...
class DiseaseGatherer (Gatherer.MultiFileGatherer):
        ...
        def buildDiseaseSorts(self):
                ...
                for row in rows:
                        term = row[termCol]
                        terms.append(term)
                terms.sort(key=symbolsort.splitter)
                seq = 0
                for term in terms:
                        seq += 1
                        dSorts[term] = seq
        ...
        def collateResults (self):
                ...
                for row in rows:
                        ...
                        if isConditional:
                            if conditionalNote not in genotypeNotes:
                                    genotypeNotes[conditionalNote] = 1 + \
                                        len(genotypeNotes)

                            if conditionalNote not in alleleNotes:
                        ...
                        if genotypeDisease not in gdData:
                                row = {
                                        'genotype' : genotype,
                                        'isHeading' : 0,
                                        'isNot' : isNot,
                        ...
                        if alleleDisease not in adData:
                                row = {
                                        'alleleKey' : allele,
                                        'isHeading' : 0,
                                        'isNot' : isNot,
                ...
                genotypeDiseaseKeys = list(gdData.keys())
                ...
                alleleDiseaseKeys = list(adData.keys())
