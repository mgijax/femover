...
def getFullTextLinks():
        ...
        if 'FULL_TEXT_LINKS' in config.__dict__:
                try:
...
class MarkerNoteGatherer (Gatherer.Gatherer):
        ...
        def collateResults (self):
                ...
                for row in self.results[0][1]:
                        ...
                        if referenceKey not in m:
                                m[referenceKey] = { noteType : note }
                        elif noteType not in m[referenceKey]:
                                m[referenceKey][noteType] = note
                        else:
                ...
                for (key, url) in list(fullTextLinks.items()):
                        if key not in m:
                ...
                referenceKeys = list(m.keys())
                referenceKeys.sort()

                for referenceKey in referenceKeys:
                        noteTypes = list(m[referenceKey].keys())
                        noteTypes.sort()
