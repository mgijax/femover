...
def getAllDiseaseModels():
        ...
        return list(DISEASE_MODEL_CACHE.values())
...
def getAdditionalModels (termKey, markerKeys):
        ...
        if termKey in TERM_TO_DISEASE_MODELS:
        ...
        # need to populate cache, then grab models for that term.
                ...
                for dm in getAllDiseaseModels():
                        ...
                        if dmTermKey in TERM_TO_DISEASE_MODELS:
...
def getDiseaseModelsByMarker (markerKey, termKey):
        ...
        if key in MARKER_TO_DISEASE_MODELS:
        ...
        for dm in list(DISEASE_MODEL_CACHE.values()):
                diseaseKey = dm.getDiseaseKey()

                for marker in dm.getMarkerKeys():
                        key2 = (marker, diseaseKey)

                        if key2 not in MARKER_TO_DISEASE_MODELS:
...
def markerKeys (desiredClassKey):
        ...
        if not CLASS_KEY_TO_MARKERS:
                for markerKey in list(MARKER_DATA_CACHE.keys()):
                        ...
                        if classKey in CLASS_KEY_TO_MARKERS:
        ...
        if desiredClassKey in CLASS_KEY_TO_MARKERS:
...
def markerType (markerKey):
        ...
        if markerKey in MARKER_DATA_CACHE:
...
def markerSortKey(a):
        # get a sort key for (symbol, causative flag)
...
class DiseaseRow:
        ...
        def getMouseGenes (self):
                ...
                if not self.mouseSorted:
                        self.mouseGenes.sort(key=markerSortKey)
        ...
        def getHumanGenes (self):
                ...
                if not self.humanSorted:
                        self.humanGenes.sort(key=markerSortKey)
...
class DiseaseDetailGatherer (Gatherer.MultiFileGatherer):
        ...
        def buildDiseaseSynonymTable (self):
                ...
                termKeys = list(synonyms.keys())
                termKeys.sort()

                for termKey in termKeys:
                        synonyms[termKey].sort (key=symbolsort.splitter)
        ...
        def collectTermMarkerAssociations (self, queryNumber, organism):
                ...
                for row in rows:
                        ...
                        if termKey in diseaseToMarkers:
        ...
        def buildRows (self,
                ...
                for classKey in list(classToDR.keys()):
                ...
                for groupType in list(groups.keys()):
                        groups[groupType].sort(key=lambda x : symbolsort.splitter(x.getSortValue()))
...
#                                   if dm.getIsNot() == 1:
                        ...
                        if drKey in additionalModelMap:
        ...
        def collateResults (self):
                ...
                for termKey in termKeys:
                        ...
                        if termKey in termToMouseMarkers:
                                cmg = termToMouseMarkers[termKey]

                        if termKey in termToHumanMarkers:
                        ...
                        allAdditionalModels = allAdditionalModels + list(additionalModelMap.items())
