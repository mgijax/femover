...
def relatedMarkerSortKey(t):
        # generate a sort key for a 3-item tuple (t) with: (chrom seq num, marker start coord, marker key),
        # including handling nulls gracefully (sort nulls first)
       
        # normal case -- no nulls
        if (t[0] != None) and (t[1] != None) and (t[2] != None):
                return t

        (chrom, coord, marker) = t
        if chrom == None:
                chrom = -99
        if coord == None:
                coord = -99
...
def getTeaserMarkers():
        ...
        alleleKeys = list(rowsPerAllele.keys())
        alleleKeys.sort()
        ...
        for group in groups:
                ...
                for allele in group:
                        ...
                        if allele not in relatedMarkers:
                                continue

                        for relatedMarker in list(relatedMarkers[allele].keys()):
                            markerList.append ( (
                                MarkerUtils.getChromosomeSeqNum(relatedMarker),
                                MarkerUtils.getStartCoord(relatedMarker),
                                relatedMarker) )

                        markerList.sort(key=relatedMarkerSortKey)
...
def getPropertyRows(startAllele, endAllele):
        ...
        for relationshipKey in list(notes.keys()):
                row = []
                for field in fields:
                        if field == keyCol:
...
def sortRowsSortKey(t):
        # generate a sort key for a 4-item tuple (t) with: (allele key, chrom seq num, marker start coord,
        # relationship row count), including handling nulls gracefully (sort nulls first)
       
        # normal case -- no nulls
        if (t[0] != None) and (t[1] != None) and (t[2] != None) and (t[3] != None):
                return t

        (alleleKey, chrom, coord, count) = t
        if alleleKey == None:
                alleleKey = -99
        if chrom == None:
                chrom = -99
        ...
        if count == None:
                count = -99
        return (alleleKey, chrom, coord, count)
...
def expandRelationshipRows(iCols, iRows):
        ...
        sortRows.sort(key=sortRowsSortKey)
